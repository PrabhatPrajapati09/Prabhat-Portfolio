...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    <div className="w-full border-b-2 border-border bg-background/70 backdrop-blur-md fixed top-0 left-0 z-50 transition-colors duration-300">
      ...
      <div className="w-full h-16 flex items-center justify-between px-4 sm:px-8 max-w-7xl mx-auto">
        ...
        <div className="text-primary-text flex items-center gap-4 sm:gap-6">
          <Link to="/" className="logo text-3xl font-['Caveat'] font-bold tracking-wider select-none">
            {"<PrabhatP/>"}
          </Link>

          <div className="hidden md:flex items-center gap-4 text-sm font-medium">
            <a href="/assets/Prabhat_Prajapati_Resume.pdf" className="resume flex items-center gap-1 cursor-pointer group hover:scale-105 transition-all duration-300 hover:text-active-button" onClick={updateResumeDownloads}>
              Resume
            ...
            <a href="https://www.linkedin.com/in/prabhat-prajapati-266423323" target="_blank" rel="noopener noreferrer" className="linkedin flex items-center gap-1 cursor-pointer group hover:scale-105 transition-all duration-300 hover:text-active-button">
              Linkedin
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" color="currentColor" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M19.0001 13V14C19.0001 17.2998 19.0001 18.9497 17.9749 19.9749C16.9498 21 15.2999 21 12.0001 21H10.0001C6.70023 21 5.05031 21 4.02519 19.9749C3.00006 18.9497 3.00006 17.2998 3.00006 14V12C3.00006 8.70017 3.00006 7.05025 4.02519 6.02513C5.05031 5 6.70023 5 10.0001 5H11.0001"></path>
                <path d="M14 3H18C19.4142 3 20.1213 3 20.5607 3.43934C21 3.87868 21 4.58579 21 6V10M20 4L11 13"></path>
        ...
        <div className="hidden md:flex items-center">
          <ul className="flex items-center gap-4 lg:gap-6">
            ...
            <li className="flex items-center gap-2 font-mono text-xs font-medium text-primary-text bg-card rounded-3xl px-3.5 py-1.5 border border-border/40 select-none shadow-sm">
            ...
            <li onClick={toggleMusic} className={`cursor-pointer transition-colors duration-300 hover:text-active-button ${isPlaying ? 'text-active-button drop-shadow' : 'text-primary-text'}`} aria-label="Toggle Background Ambient Stream">
            ...
            <li className="cursor-pointer text-primary-text hover:text-active-button transition-colors" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
              {theme === 'dark' ? (
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" color="currentColor" fill="none" stroke="currentColor" strokeWidth="1.5">
            ...
            <li className="cursor-pointer text-primary-text hover:text-active-button transition-colors">
            ...
            {isAdmin && (
              <>
                <div className="w-[1.5px] h-6 bg-border rounded-full mx-1"></div>

                {!isDashboard ? (
                  <>
                    <li className="cursor-pointer bg-card border border-border/60 hover:border-active-button px-3 py-1.5 rounded-xl transition-all">
                    ...
                    <li className="cursor-pointer bg-card border border-border/60 hover:text-active-button px-4 py-1.5 rounded-xl transition-all text-xs font-semibold text-primary-text">
                      <Link to="/admin">Dashboard</Link>
                    </li>
                  </>
                ) : (
                  <li className="cursor-pointer bg-active-button hover:bg-active-button/90 text-white px-4 py-1.5 rounded-xl transition-all text-xs font-semibold shadow-sm">
                    <Link to="/">View Live Site</Link>
                  </li>
                )}

                <li className="cursor-pointer text-secondary-text hover:text-red-500 transition-colors">
        ...
        <div className="flex md:hidden items-center gap-4 text-primary-text">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Toggle Navigation Stack Menu">
      ...
      {isMobileMenuOpen && (
        <div className="w-full bg-background border-t border-border flex flex-col md:hidden p-5 gap-4 shadow-xl animate-fade-in-up font-medium text-center text-primary-text">
          <div className="flex justify-center gap-6 text-sm border-b border-border/40 pb-3">
            <div onClick={() => { updateResumeDownloads(); setIsMobileMenuOpen(false); }} className="cursor-pointer hover:text-active-button">Resume</div>
            <a href="https://www.linkedin.com/in/prabhat-prajapati-266423323" target="_blank" rel="noopener noreferrer" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-active-button">LinkedIn</a>
            <a href="https://github.com/PrabhatPrajapati09" target="_blank" rel="noopener noreferrer" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-active-button">GitHub</a>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 py-2">
            ...
            <button onClick={toggleMusic} className={`p-1.5 rounded-xl border border-border/60 ${isPlaying ? 'text-active-button' : 'text-secondary-text'}`}>
              Music: {isPlaying ? "ON" : "OFF"}
            </button>
            
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-1.5 rounded-xl border border-border/60 text-secondary-text">
          ...
          {isAdmin && (
            <div className="flex flex-col gap-3 pt-3 border-t border-border/40 text-xs">
              {!isDashboard ? (
                <>
                  <button onClick={() => { toggleDevMode(); setIsMobileMenuOpen(false); }} className={`py-2 rounded-lg bg-card border ${devMode ? "text-active-button border-active-button/40" : "text-secondary-text border-border"}`}>
                    {devMode ? "Dev Mode: Active" : "Dev Mode: Offline"}
                  </button>
                  <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)} className="py-2 bg-card border border-border rounded-lg text-primary-text font-semibold">
                    Dashboard Overview
                  </Link>
                </>
              ) : (
                <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="py-2 bg-active-button text-white rounded-lg font-semibold shadow-sm">
                  Exit to Live Site
                </Link>
              )}
              <Link to="/" onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="py-2 border border-red-500/30 text-red-500 rounded-lg bg-red-500/5">
                Logout Session
              </Link>
            </div>
          )}
